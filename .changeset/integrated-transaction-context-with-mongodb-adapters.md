...
- `MongodbCircuitBreakerStorageAdapter` and `MongodbRateLimiterStorageAdapter` now require `database: ITransactionContext<Db, ClientSession>` instead of a plain `Db`, because their operations rely on transactions to stay correct. A plain `Db` is no longer accepted, and the previous `client: MongoClient` setting has been removed. Their `transaction(fn)` method runs the callback through the provided transaction context with `REQUIRED` propagation.
